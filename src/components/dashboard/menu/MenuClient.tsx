  ...
  return (
    <div className="space-y-8">
      ...
      <motion.div
        ...
        className="flex flex-col lg:flex-row md:items-center gap-4 bg-white dark:bg-white/5 p-2 rounded-[24px] shadow-sm border border-slate-100 dark:border-white/5"
      >
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between pt-8 border-t border-slate-100 dark:border-white/5">
          <p className="text-sm text-slate-500 font-medium">
            {g('showing')} <span className="text-slate-900 dark:text-white font-bold">{items.length}</span> {g('of')} <span className="text-slate-900 dark:text-white font-bold">{total}</span> {g('items')}
          </p>
          <div className="flex items-center gap-2">
            <button
              onClick={() => setPage(p => Math.max(1, p - 1))}
              disabled={page === 1 || isFetching}
              className="p-2 rounded-xl bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 text-slate-600 dark:text-slate-400 disabled:opacity-50 hover:border-primary hover:text-primary transition-all"
            >
